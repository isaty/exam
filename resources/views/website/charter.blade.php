<div class="center-head row">
    <div class="col-12 col-lg-10 offset-lg-1 py-5 my-5">

        <div class="center-carousel">

            <div id="charter" class="carousel slide" data-ride="carousel">

                <!-- Indicators -->
                <ul class="carousel-indicators">
                    <li data-target="#charter" data-slide-to="0" class="active"></li>
                    <li data-target="#charter" data-slide-to="1"></li>
                    <li data-target="#charter" data-slide-to="2"></li>
                </ul>

                <!-- The slideshow -->
                <div class="carousel-inner text-center" style="min-height: 400px">
                    <div class="carousel-item active">
                        <figure class="hod-image">
                            <img src="./assets/images/faculty/1.jpg">
                        </figure>
                        <div class="member-name">Dr. Sunita Yadav</div>
                        <div class="member-data">HOD CSE</div>
                    </div>
                    <div class="carousel-item">
                        <div class="row">
                            <div class="col-6">
                                <figure class="member-image">
                                    <img src="./assets/images/faculty/2.jpg">
                                </figure>
                                <h4 class="txt-theme mt-5 pt-3">Dr. Arun Yadav</h4>
                                <p class="txt-black">Professor CSE</p>
                            </div>
                            <div class="col-6">
                                <figure class="member-image">
                                    <img src="./assets/images/faculty/3.jpg">
                                </figure>
                                <h4 class="txt-theme mt-5 pt-3">Dr. Sonam Gupta</h4>
                                <p class="txt-black">Professor</p>
                            </div>
                        </div>
                    </div>
                    <div class="carousel-item">
                        <div class="row">
                            <div class="col-6">
                                <figure class="member-image">
                                    <img src="./assets/images/faculty/4.jpg">
                                </figure>
                                <h4 class="txt-theme mt-5 pt-3">Deepak Rai</h4>
                                <p class="txt-black">Assistant Professor</p>
                                <p class="txt-black"></p>
                            </div>
                            <div class="col-6">
                                <figure class="member-image">
                                    <img src="./assets/images/faculty/5.jpg">
                                </figure>
                                <h4 class="txt-theme mt-5 pt-3">Abhishek Sir</h4>
                                <p class="txt-black">Lab Assistant</p>
                            </div>
                        </div>
                    </div>
                </div>


                <!-- Left and right controls -->
<!--                <a class="carousel-control-prev" href="#events" data-slide="prev">-->
<!--                    <span class="carousel-control-prev-icon"></span>-->
<!--                </a>-->
<!--                <a class="carousel-control-next" href="#events" data-slide="next">-->
<!--                    <span class="carousel-control-next-icon"></span>-->
<!--                </a>-->
            </div>

        </div>

    </div>
</div>
